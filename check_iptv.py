...
# 原始标题 → 大类映射（你的 live.txt 里所有标题都覆盖到）
TITLE_MAP = {
    "直播":           "国际源",
    "其他":           "其他",
    "GitHub源":       "GitHub源",
...
def classify(url, title="未分类"):
    ...
    if 'migu' in low or 'miguvideo' in low:
        return "国内源"
    if 't.freetv.fun' in low or 'live.zbds' in low or 'live.hacks' in low or 'live.zhoujie' in low:
        return "国内源"
    if 'freetv' in low or '850930' in low:
        return "国内源"
    if 'ibert.me' in low:
        return "国内源"
    if 'kstore' in low or '7so.top' in low or 'cloud.7' in low:
        return "其他"
...
def main():
    ...
    # ━━━ 写文件 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # live_ok.txt —— 只用 CAT_ORDER 里的大类名
    ...
    print(f"💾 已生成:")
    print(f"   live_ok.txt     ← {total_ok} 条（6 大类固定输出）")
